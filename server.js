...
var server = http.createServer(function(request, response){
    ...
    if(path === '/'){
        ...
        const page1 = fs.readFileSync('db/1.json')
        ...
        response.end()
    }else if(path ==='/minor.js'){
        response.statusCode = 200
        response.setHeader('Content-Type', 'text/javascript;charset=utf-8')
        response.write(fs.readFileSync('src/minor.js'))
        response.end()
    }else if(path ==='/3.html'){
        response.statusCode = 200
        response.setHeader('Content-Type', 'text/html;charset=utf-8')
        ...
        response.end()
    }else if(path ==='/text.json'){
        response.statusCode = 200
        response.setHeader('Content-Type', 'text/json;charset=utf-8')
        response.write(fs.readFileSync('src/text.json'))
        response.end()
    }else if(path ==='/page2'){
        response.statusCode = 200
        response.setHeader('Content-Type', 'text/json;charset=utf-8')
